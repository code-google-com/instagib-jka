...
//---------------------------------------------------------
{
	...
	int			traces = Q3_INFINITE; //DISRUPTOR_ALT_TRACES;
	qboolean	fullCharge = qfalse;
	//[Instagib]
	vec3_t		teleportDest;
	vec3_t		teleportAngles;
	int paralyseDuration;

	//damage = DISRUPTOR_ALT_DAMAGE-30;
	damage = Q3_INFINITE;
	//[/Instagib]

	if ( ent->client->pers.gunMode == 2 ) {
		traces = 1;
	}
	//[/Instagib]
	...
	//[Instagib]
	else if ( count >= maxCount && ent->client->ps.zoomMode == 1 )
	...
	for (i = 0; i < traces; i++ )
	{
		...
		//[Instagib]
		if (traceEnt == ent) {
			break; // lolololol don't kill urself it is stupid :o
		}
		//[/Instagib]
		...
		if ( render_impact ) 
		{
						...
						|| traceEnt->s.eType == ET_MOVER )
				 {
					if ( traceEnt->takedamage )
					{
						G_Damage( traceEnt, ent, ent, forward, tr.endpos, damage, 
								DAMAGE_NO_KNOCKBACK, MOD_DISRUPTOR);//_SNIPER );
			...
			if ( traceEnt->takedamage )
			{
				...
				//[Instagib]
				if ( ent->client->pers.gunMode == 1 && fullCharge ) { // if fully charged : dismember it!
					traceEnt->fullDismember = qtrue;
				}

				if ( ent->client->pers.gunMode != 2 ) { // not on paralyse mode?
					// Kill it. Now!
					...
					//Paralyse mode : knocking down and immobilise
					G_Knockdown( traceEnt );
					paralyseDuration = Q_irand(4000, 7000);
					traceEnt->client->paralyseTimer = level.time + paralyseDuration;
					traceEnt->client->ps.electrifyTime = level.time + paralyseDuration;
					if (ent->client->ps.zoomMode) {
						// if charged, push it.
						VectorMA( traceEnt->client->ps.velocity, count*30, forward, traceEnt->client->ps.velocity );
					}
				}

				/*if (traceEnt->client && preHealth > 0 && traceEnt->health <= 0 && fullCharge &&
					G_CanDisruptify(traceEnt))*/
				if (traceEnt->client && preHealth > 0 && traceEnt->health <= 0 && ent->client->pers.gunMode == 0 &&
					!fullCharge && G_CanDisruptify(traceEnt))
				//[/Instagib]
				{ //was killed by a fully charged sniper shot, so disintegrate
					...
					VectorClear( teleportAngles );
					teleportDest[0] = 0;
					teleportDest[1] = 0;
					teleportDest[2] = 0;
					teleportAngles[YAW] = 0;
					TeleportPlayer(traceEnt, teleportDest, teleportAngles);
				}
