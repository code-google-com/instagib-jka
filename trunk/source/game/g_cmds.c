...
void Cmd_Kill_f( gentity_t *ent ) {
	...
	if (ent->client->paralyseTimer > level.time) {
		return;
	}
	...
	if ( ent->client->ps.weapon != WP_SABER )
	{
		...
		if ( ent->client->ps.weapon == WP_DISRUPTOR ) {
			if ( ent->client->pers.gunMode == 2 ) {
				ent->client->pers.gunMode = 1;
				trap_SendServerCommand( ent-g_entities, va("print \" %s\n\"", "Gun mode set to" S_COLOR_CYAN" DISMEMBERMENT"));
			} else if ( ent->client->pers.gunMode == 0 ) {
				ent->client->pers.gunMode = 2;
				trap_SendServerCommand( ent-g_entities, va("print \" %s\n\"", "Gun mode set to" S_COLOR_GREEN " PARALYSE"));
			} else {
				ent->client->pers.gunMode = 0;
				trap_SendServerCommand( ent-g_entities, va("print \" %s\n\"", "Gun mode set to" S_COLOR_RED " DISINTEGRATION"));
			}
			G_Sound(ent, CHAN_WEAPON, G_SoundIndex("sound/interface/pickup_battery.mp3"));
		}
		//[/Instagib]
        return;
	}
